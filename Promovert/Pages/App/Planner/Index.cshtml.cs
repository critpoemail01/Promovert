...
namespace Promovert.Pages.App.Planner;
...
public class IndexModel : PageModel
{
    ...
    public class PlannerInput
    {
        ...
        public List<string> Platforms { get; set; } = ["LinkedIn", "Instagram"];
    ...
    public async Task<IActionResult> OnPostSuggestAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            SuggestionMessage = $"Detected: {suggestion.DetectedApplicationType}. Review the suggested brief before generating the campaign.";
            ModelState.Clear();
        }
    ...
    public async Task<IActionResult> OnPostUseTemplateAsync(string templateKey)
    {
        ...
        ActiveCampaignTemplateKey = template.Key;
        Input.SelectedTemplateKey = template.Key;
        SuggestionMessage = $"{template.Sector}: '{template.Title}' applied. Review the structured campaign before generating.";
        await LoadPlansAsync();
        return Page();
    }
    ...
    public async Task<IActionResult> OnPostUseCustomAsync()
    {
        ...
        Input.DetectedApplicationType = "Custom";
        Input.SelectedTemplateKey = "custom";
        ActiveCampaignTemplateKey = "custom";
        SuggestionMessage = "Custom campaign selected. Define goal, audience, offer, channels and cadence before generating.";
        await LoadPlansAsync();
        return Page();
    }
    ...
    public async Task<IActionResult> OnPostUseMvpAsync()
    {
        ...
        Input.CampaignGoal = string.IsNullOrWhiteSpace(Input.CampaignGoal) || Input.CampaignGoal is "subscriptions" or "subscriptions"
            ? "qualified leads"
            : Input.CampaignGoal;
        ...
        SuggestionMessage = "MVP funnel applied: 14 days, 5 LinkedIn posts, 5 Instagram posts, 3 emails, landing page, UTM links, approval and dashboard.";
        await LoadPlansAsync();
        return Page();
    }
    ...
    private async Task<IActionResult> SuggestLeadEmailsAsync(bool searchOnline, CancellationToken cancellationToken)
    {
        ...
        if (result.Emails.Count > 0)
        {
            ...
            LeadDiscoveryMessage = searchOnline
                ? $"{result.Emails.Count} email audience{(result.Emails.Count == 1 ? "" : "s")} suggested from company pages and added for review."
                : $"{result.Emails.Count} email audience{(result.Emails.Count == 1 ? "" : "s")} found and added to the review list.";
        }
        else
        {
            LeadDiscoveryMessage = searchOnline
                ? "No public emails were found automatically. Review the prospecting searches or write emails manually."
                : "Prospecting searches prepared. Use the results to validate companies and contacts before sending.";
        }
    ...
    public async Task<IActionResult> OnPostGenerateAsync()
    {
        ...
        plan.BusinessDna = plan.CrmLeadSourceCount > 0
            ? Clip($"{plan.BusinessDna} CRM focus: {plan.CrmLeadSourceCount} email{(plan.CrmLeadSourceCount == 1 ? "" : "s")} from imported leads with filter '{plan.CrmLeadFilter ?? "all CRM leads"}'.", 1000)
            : plan.BusinessDna;
    ...
    private async Task LoadNextCampaignSourceAsync(int sourcePlanId)
    {
        ...
        SuggestionMessage = "Next campaign prepared from the previous report and captured leads.";
    }
    ...
    private void ApplyTemplate(SectorCampaignRecommendation template)
    {
        ...
        Input.CompanyOrIdea = Clip(string.IsNullOrWhiteSpace(Input.CompanyOrIdea)
            ? $"Campaign {template.Sector}: {template.Strategy}"
    ...
    private void ValidateLocation()
    {
        if (Input.AudienceLocationScope == "Country" && string.IsNullOrWhiteSpace(Input.AudienceCountry))
            ModelState.AddModelError("Input.AudienceCountry", "Add the audience country for this campaign.");

        if (Input.AudienceLocationScope != "City")
            return;

        if (string.IsNullOrWhiteSpace(Input.AudienceCity))
            ModelState.AddModelError("Input.AudienceCity", "Add the city or click the map to define the campaign center.");
