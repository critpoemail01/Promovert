...
public class CrmLeadImportServiceTests
{
    ...
    public void Parse_SupportsPortugueseSemicolonHeaders()
    {
        ...
        var rows = service.Parse("""
            nome;correio eletrónico;empresa;cargo;indústria;país;cidade;fase;telemóvel;observações
            João;joao@example.com;Empresa Lda;Diretor;Indústria;Portugal;Lisboa;Novo;+351900000000;Pedir demo
            """, "Pipedrive");

        var row = Assert.Single(rows);
        Assert.Equal("João", row.ContactName);
        Assert.Equal("Empresa Lda", row.CompanyName);
        Assert.Equal("Diretor", row.Role);
        Assert.Equal("Indústria", row.Industry);
        Assert.Equal("Portugal", row.Country);
        Assert.Equal("+351900000000", row.Phone);
        Assert.Equal("Novo", row.Stage);
        Assert.Equal("Pedir demo", row.Notes);
    }
